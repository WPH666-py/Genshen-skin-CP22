...
def pkg_root():
    """包目录 .../genshen_skin_cp22(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp22.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #f3eef0;
  --bg-layer-3: #e4dcdf;
  --label-primary: #2a2130;
  --label-secondary: #524660;
  --label-tertiary: #857a92;
  --brand-primary: #5c78b0;
  --accent: #5c78b0;
  --primary: #5c78b0;
  --interactive-bg-hover-solid: #e2e6f2;
  --interactive-bg-active: #c9d3ea;
  --border-l1: #e6e0ea;
  --border-l2: #cdc4d4;
  --border: #cdc4d4;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP22</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP22", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP22", mascot.get("dark")),
    }
