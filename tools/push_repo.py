...
SKIP_DIRS = {".git", "__pycache__", "build", ".eggs",
             "genshen_skin_cp22.egg-info", "node_modules", ".vscode-test"}
SKIP_SUFFIX = (".pyc", ".pyo", ".bak", ".orig", ".rej")
SKIP_FILES = {".mcp.json"}
SKIP_PREFIX = ("genshen_skin_cp22-",)
...
def collect(root):
    ...
    build 期的 genshen_skin_cp22-0.1.0/ 源码副本、__pycache__ 等一律排除。
    """
    out = []
...
def post_retry(gh, path, payload, what, tries=6):
    """GitHub 偶发 400 "malformed request" / 5xx / 超时, 与内容无关。

    Git Data API 的每一步都可能碰到, 所以统一退避重试。
    """
    last = None
    for attempt in range(tries):
        try:
            return gh.post(path, payload)
        except RuntimeError as e:
            last = e
            msg = str(e)
            transient = ("malformed request" in msg or " 5" in msg
                         or "timed out" in msg or "timed" in msg)
            if transient and attempt < tries - 1:
                wait = 2 * (attempt + 1)
                log("      %s 失败(%s), %ds 后重试 (%d/%d) ..."
                    % (what, msg.split("->")[0].strip()[-4:], wait,
                       attempt + 1, tries - 1))
                time.sleep(wait)
                continue
            raise
    raise RuntimeError("%s 最终失败: %s" % (what, last))
...
def main():
    ...
    ap.add_argument("--message", default="feat: 原神CP壁纸套件2 · 荧×达达利亚")
    ...
    # 1) 为达达利亚仓库创建首个 commit, 需要先有一个 tree。
    ...
    for i, (rel, abs_p, is_text) in enumerate(files, 1):
        ...
        if is_text:
            ...
            payload = {"content": base64.b64encode(raw).decode("ascii"),
                       "encoding": "base64"}
        blob = post_retry(gh, "/repos/%s/git/blobs" % args.repo, payload, rel)
    ...
    for attempt in range(6):
        try:
            new_tree = post_retry(gh, "/repos/%s/git/trees" % args.repo, {"tree": tree}, "创建 tree")
            break
        except RuntimeError as e:
            msg = str(e)
    ...
    try:
        ...
        log("      达达利亚仓库, 创建首个 commit")
    commit = post_retry(gh, "/repos/%s/git/commits" % args.repo, commit_payload, "创建 commit")
    log("      commit = %s" % commit["sha"])

    # 4) ref
    log("[4/4] 更新分支 %s ..." % args.branch)
    try:
        post_retry(gh, "/repos/%s/git/refs" % args.repo,
                   {"ref": "refs/heads/" + args.branch, "sha": commit["sha"]}, "创建 ref")
