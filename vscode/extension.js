// 原神 CP 壁纸套件 22 — 荧 × 达达利亚 · 单张素材三种呈现 (VSCode / Trae / CodeX / Cursor / Windsurf)
...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP22.git';
const EXT_ID = 'wp666.genshen-skin-cp22';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp22').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp22').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP22'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP22') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshen_skin_cp22', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshen_skin_cp22.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshen_skin_cp22.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp22' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP22') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshen_skin_cp22.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
// 本套件只有一张素材(荧 × 达达利亚 双人合影), 因此画廊给的是同一张的
// 三种呈现方式。ids 必须与 characters/cp22_pair.py 的 MODES 一致。
const CARDS = [
  { mode: 'single1', t: '婚纱 · 卡片', img: 'thumb-single1.png', d: '模糊背景 + 居中卡片' },
  { mode: 'cover1', t: '婚纱 · 满屏', img: 'thumb-cover1.png', d: 'cover 铺满整屏' },
  { mode: 'showall1', t: '婚纱 · 完整', img: 'thumb-showall1.png', d: '不裁切, 两侧留边' }
];
...
function renderHtml(thumb, hasPython) {
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>' +
    ...
    '</div>' +
    '<div class="foot">原神 CP 壁纸套件 22 · 荧 × 达达利亚 v0.1.0<br/>' +
...
function activate(ctx) {
  ...
  reg('genshencp22.setCard', () => runCli(['card'], '已切换: 卡片式'));
  reg('genshencp22.setCover', () => runCli(['cover'], '已切换: 满屏'));
  reg('genshencp22.setShowall', () => runCli(['showall'], '已切换: 完整不裁'));
  reg('genshencp22.setRandom', () => runCli(['random'], '已随机换一种!'));
  reg('genshencp22.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshencp22.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshencp22.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp22.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP22 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp22.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp22')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp22.gallery', new GalleryProvider(), {
