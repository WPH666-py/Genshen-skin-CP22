...
try:
    from genshen_skin_cp22.engine import skin_core as sc
...
def make_thumbs():
    ...
    # 拼版总览图: 按实际样式数量排成一行(本套件 3 个), 不留达达利亚行
    modes_n = len(modes)
...
def make_icon():
    ...
    # 渐变底: 上矢车菊蓝 -> 下深紫墨(与皮肤 accent #5c78b0 同源)
    top = (92, 120, 176)     # #5c78b0 蓝玫瑰的矢车菊蓝
    bottom = (42, 33, 48)   # #2a2130 深紫墨
    for y in range(S):
        t = y / (S - 1)
